...
rule checkStartedToStateTransition(method f, uint256 meetingId) {
	...
	assert (stateAfter == 2 || stateAfter == 3), "the status of the meeting changed from STARTED to an invalid state";
	assert (stateAfter == 3 => f.selector == endMeeting(uint256).selector), "the status of the meeting changed from STARTED to ENDED through a function other then endMeeting()";
}
...
rule checkPendingToCancelledOrStarted(method f, uint256 meetingId) {
	...
	assert (stateAfter == 1 || stateAfter == 2 || stateAfter == 4), "invalidation of the state machine";
	assert (stateAfter == 2) => f.selector == startMeeting(uint256).selector, "the status of the meeting changed from PENDING to STARTED through a function other then startMeeting()";
	assert (stateAfter == 4) => f.selector == cancelMeeting(uint256).selector, "the status of the meeting changed from PENDING to CANCELLED through a function other then cancelMeeting()";
}
...
rule checkCancelledOrFinished(method f, uint256 meetingId) {
	...
	f(e, args);
    uint8 stateAfter = getStateById(meetingId);
	assert stateAfter == stateBefore, "Cannot updated Cancelled / Finished meetings";
}
